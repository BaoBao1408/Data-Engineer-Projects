...
def generate_emergency_incident_data(device_id, incident_id, location):
    return {
        'id': uuid.uuid4(),
        'deviceId': device_id,
...
def simulator_journey(producer, device_id):
    while True:
        ...
        if (vehicle_data['location'][0]) >= BIRMINGHAM_COORDINATES['latitude'] \
            ...
            print(
                f"✅ Message delivered to {VEHICLE_TOPIC}, {GPS_TOPIC}, {TRAFFIC_TOPIC}, {WEATHER_TOPIC}, {EMERGENCY_TOPIC}")
            # break
...
if __name__ == "__main__":
    ...
    try:
        simulator_journey(producer, 'Vehicle-Morgan-123')

    except KeyboardInterrupt:
        print("Simulator ended by user")
